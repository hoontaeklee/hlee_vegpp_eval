'''
- detrend output variables of a sindbad run
'''
...
def get_trends(dat_full, nlat=180, nlon=360):
    # dat_full: numpy array with the dimension of (time, lat, lon)
...
def detrend_sindbad_output(path_expOutput):
    # detrend a xr.dataset
    
    # load missing timesteps of GRACE
    path_gra_invalid_dates = '/Net/Groups/BGI/people/hlee/data/twsNeeTimeseriesScatter/fromMjCloneCodeBySk/GRACE_TWS_JPL_RL06Mv1CRIv01_invalid_dates_200201_201712.npy'
    ...
    for f in np.arange(0, len(files_var)):  # range(len(files_var)):
        ...
        if _vname not in ['NEE']:
            continue

        if type(dict_target_var[_vname])==tuple:  # in case the variable has only one file and time period
            date_start = dict_target_var[_vname][0]
            date_end = dict_target_var[_vname][1]
            ...
            # GRACE --> apply the same missing dates of GRACE, use anomaly before calculate MSC and IAV
            if _vname == 'wTotal':
                ds = xr.where(~gra_invalid_dates_3d, ds, np.nan)

            # if zix diminsion exists, sum along them
            if sum(['level' in ele for ele in list(ds.dims)]) != 0:
                # only works if only one zix dimension exists
                ds = ds.sum(dim=[ele for ele in list(ds.dims) if 'level' in ele])
            ...
            save_name = os.path.join(path_out, f'{runname}_{_vname}.nc')
        ...
        if type(dict_target_var[_vname])==list:  # in case the variable has two different files and time periods
            for i in range(len(dict_target_var[_vname])):
                ...
                if _vname=='NEE':
                    ...
                    fire['time'] = ds['time']
                    fire = fire.sortby('lat', ascending=True)
                    
                    ds[_vname] = xr.where(np.isnan(fire['C_Emissions']), ds[_vname], ds[_vname] + fire['C_Emissions'])  # add fire emission
                ...
                if _vname=='NEE':
                    save_name = os.path.join(path_out, f'{runname}_{_vname+date_start[:4]+date_end[:4]}_withFireC.nc')
