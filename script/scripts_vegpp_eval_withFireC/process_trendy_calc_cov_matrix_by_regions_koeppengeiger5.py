...
def process_trendy_members_cov_matrix_by_regions_koeppengeiger5():
    ...
    for v in range(len(list(dict_var.keys()))):
        ...
        path_trd_var = os.path.join(path_trddet, f'trendyv9_{trd_exp}_{vname}-regionalMSCIAV_{vperiod}_koeppengeiger5.nc')
        ...
        np.savez(
                os.path.join(path_trddet, 'cov_norm', f'koeppengeiger5_region_cov_norm_{trd_exp}_{vname}_{vperiod}_EnsMedian.npz'),
        ...
        np.savez(
            os.path.join(path_trddet, 'cov_norm', f'koeppengeiger5_region_cov_norm_{trd_exp}_{vname}_{vperiod}_members.npz'),
