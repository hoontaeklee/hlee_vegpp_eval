...
def calc_vegpp_cov_matrix_by_regions_koeppengeiger5(path_expOutput):
    ...
    for i, var_to_diag in enumerate(vars_to_diag):
        ...
        if var_to_diag not in ['NEE']:
            continue

        #%% load data
        nee_label = '20152019' if var_to_diag=='NEE' else ''
        dir_det_sin = path_expOutput
        file_det_sin = path_expOutput.split('/')[-2] + '_' + var_to_diag + nee_label + '_withFireC.nc'
        ...
        np.savez(
            os.path.join(path_expOutput, f'koeppengeiger5_region_cov_norm_{var_to_diag}_withFireC.npz'),
