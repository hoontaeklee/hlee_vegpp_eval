...
def process_trendy_calc_regional_msciav_koeppengeiger5():
    ...
    for p in range(len(dict_period['date_start'])):
        ...
        dict_src = {
            # 'gpp': {
            #     'path': os.path.join(path_trendy, f'trendyv9_{trd_exp}_gpp-det_{date_label}.nc'),
            #     'period': (date_start, date_end)
            # },
            # 'nee': {
            #     'path': os.path.join(path_trendy, f'trendyv9_{trd_exp}_nee-det_{date_label}.nc'),
            #     'period': (date_start, date_end)
            # },
            # 'reco': {
            #     'path': os.path.join(path_trendy, f'trendyv9_{trd_exp}_reco-det_{date_label}.nc'),
            #     'period': (date_start, date_end)
            # },
            'nbp': {
                'path': os.path.join(path_trendy, f'trendyv9_{trd_exp}_nbp-det_{date_label}.nc'),
                'period': (date_start, date_end)
            }
        }
        ...
        for v in range(len(dict_src)):
            ...
            # save
            file_out = os.path.join(path_trendy, f'trendyv9_{trd_exp}_{_vname}-regionalMSCIAV_{date_label}_koeppengeiger5.nc')
