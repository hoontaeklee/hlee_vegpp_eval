...
def plot_figa11_msc_contrib_by_regions_koeppengeiger5ns(path_expOutput):
    ...
    list_rorder = list(np.arange(rcnt)+1)
    ...
    # the ensemble median
    path_npz_med_flx = os.path.join(path_flx, 'koeppengeiger5ns_region_cov_norm_GPP_EnsMedian.npz')
    ar_cov_gpp_msc_kg_med_flx = np.load(path_npz_med_flx)['ar_cov_msc']

    # members
    path_npz_mem_flx = os.path.join(path_flx, 'koeppengeiger5ns_region_cov_norm_GPP_members.npz')
    ...
    bx_kg.set_xticklabels(rnames_rm, rotation=90)
    jt_kg = sns.stripplot(
        ax=ax,
        data=df_mem_nee.loc[df_mem_nee['region_group']=='kg5'].reset_index(),
        x='region',
    ...
    save_name = os.path.join(path_out, f"figa11_cov_msc_region_koeppengeiger5ns.png")
    fig.savefig(
        save_name,
        dpi=600,
        bbox_inches='tight',
...
if __name__ == '__main__':
    plot_figa11_msc_contrib_by_regions_koeppengeiger5ns(path_expOutput=argv[1])
