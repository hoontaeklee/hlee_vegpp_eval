'''
figure a05: nee seasonal variability across regions
...
def plot_figa05_nee_msc_by_regions_in_PgCperyr(path_expOutput):
    ...
    for i in range(len(list_var)):
        ...
        ar_obs_glo_val = ds_oco2_glo_med['net_med_nf_msc'].data / 365
        ar_obs_glo_unc = ds_oco2_glo_mad['net_mad_nf_msc'].data / 365 * 1.25
        ar_obs_reg_val = ds_oco2_reg_med['net_med_nf_msc'].data / 365
        ar_obs_reg_unc = ds_oco2_reg_mad['net_mad_nf_msc'].data / 365 * 1.25
        ...
        ar_obs_glo_val = ar_obs_glo_val * np.nansum(area_msk) * 1000 * 1000 * 365 * 1e-15
        ar_obs_glo_unc = ar_obs_glo_unc * np.nansum(area_msk) * 1000 * 1000 * 365 * 1e-15
        ar_obs_reg_val = (ar_obs_reg_val.T * area_tc['common_nonnan_pixels'].data * 1000 * 1000 * 365 * 1e-15).T
        ar_obs_reg_unc = (ar_obs_reg_unc.T * area_tc['common_nonnan_pixels'].data * 1000 * 1000 * 365 * 1e-15).T
    ...
    fig.text(-0.04, 0.51, 'NEE (PgC yr-1)', va='center', rotation='vertical', fontsize=fsize_suplab)
    ...
    fig.savefig(
        os.path.join(path_out, f"figa05_nee_msc_by_regions_in_PgCyr-1.png"),
        dpi=600,
        transparent=False,
        bbox_inches='tight'
    )
...
if __name__ == '__main__':
    plot_figa05_nee_msc_by_regions_in_PgCperyr(path_expOutput=argv[1])
# %%
