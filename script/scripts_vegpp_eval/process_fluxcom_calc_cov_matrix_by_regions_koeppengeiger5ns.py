...
def process_fluxcom_calc_cov_matrix_by_regions_koeppengeiger5ns():
    '''
    '''
    ...
    path_det_ens = os.path.join(path_bgi, 'people/hlee/data/FLUXCOM/members_detrended/region_koeppengeiger5ns')
    path_median_msc = os.path.join(path_det_ens, f'EnsMedian_{vname}_RS_METEO-NONE_Koeppengeiger5nsRegions_2002-2015_studyArea_msc.nc')
    path_median_det = os.path.join(path_det_ens, f'EnsMedian_{vname}_RS_METEO-NONE_Koeppengeiger5nsRegions_2002-2015_studyArea_det_fromRunningMean.nc')
    ...
    np.savez(
            os.path.join(path_out, f'koeppengeiger5ns_region_cov_norm_{vname}_EnsMedian.npz'),
    ...
    for f in range(nmembers):
        ...
        ds_det_tc = apply_koeppengeiger5ns_mask(
            dsin=ds_det_msk,
            pathlf=path_lf,
            varlf=vname_lf,
            faclf=fac_lf,
        ...
        np.savez(
            os.path.join(path_out, f'koeppengeiger5ns_region_cov_norm_{vname}_{membername}.npz'),
    ...
    np.savez(
        os.path.join(path_out, f'koeppengeiger5ns_region_cov_norm_{vname}_members.npz'),
        ar_cov_msc=ar_cov_norm_msc_mem_stack,
        ar_cov_det=ar_cov_norm_det_mem_stack
    )

if __name__ == '__main__':
    process_fluxcom_calc_cov_matrix_by_regions_koeppengeiger5ns()