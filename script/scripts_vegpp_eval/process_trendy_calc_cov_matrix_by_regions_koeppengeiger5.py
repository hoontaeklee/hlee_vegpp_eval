...
def process_trendy_members_cov_matrix_by_regions_koeppengeiger5():
    ...
    for v in range(len(list(dict_var.keys()))):
        ...
        np.savez(
                os.path.join(path_trddet, 'cov_norm', f'koeppengeiger5_region_cov_norm_{vname}_{vperiod}_EnsMedian.npz'),
        ...
        np.savez(
            os.path.join(path_trddet, 'cov_norm', f'koeppengeiger5_region_cov_norm_{vname}_{vperiod}_members.npz'),
