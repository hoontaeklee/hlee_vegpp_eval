...
for f in range(len(files)):
    ...
    ds_nf = ds_nf.rename({'latitude':'lat', 'longitude':'lon'})
    ds_nf['net'] = xr.where(np.isnan(fire['C_Emissions']), ds_nf['net'], ds_nf['net'] - fire['C_Emissions'])  # subtract fire emission

    ds_det = detrend_dataset(ds_orig=ds_nf, list_var=['net'], use_anomaly=False)
    ds_det.to_netcdf(os.path.join(path_out, files[f]).replace('.nc4', '_GFED4FireRemoved_det_2015_2019.nc'))