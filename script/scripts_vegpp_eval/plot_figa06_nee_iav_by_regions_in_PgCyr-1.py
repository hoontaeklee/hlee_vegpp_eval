...
def plot_figa06_nee_iav_by_regions_in_PgCperyr(path_expOutput):
    ...
    for i in range(len(list_var)):
        ...
        ndaysperyear = np.repeat(np.array([pd.Timestamp(e, 12, 31).dayofyear for e in np.arange(2015, 2019+1)]), 12)
        _sin = _sin * np.nansum(area_msk) * 1000 * 1000 * ndaysperyear * 1e-15
        _nc_sin_tc_rm[_vname_in_sin] = _nc_sin_tc_rm[_vname_in_sin] * area_tc['common_nonnan_pixels'] * 1000 * 1000 * ndaysperyear * 1e-15
        ar_obs_glo_val = ar_obs_glo_val * np.nansum(area_msk) * 1000 * 1000 * ndaysperyear * 1e-15
        ar_obs_glo_unc = ar_obs_glo_unc * np.nansum(area_msk) * 1000 * 1000 * ndaysperyear * 1e-15
        ar_obs_reg_val = (ar_obs_reg_val.T * area_tc['common_nonnan_pixels'].data).T * 1000 * 1000 * ndaysperyear * 1e-15
        ar_obs_reg_unc = (ar_obs_reg_unc.T * area_tc['common_nonnan_pixels'].data).T * 1000 * 1000 * ndaysperyear * 1e-15
        ar_trd_glo_val = ar_trd_glo_val * np.nansum(area_msk) * 1000 * 1000 * ndaysperyear * 1e-15
        ar_trd_glo_unc = ar_trd_glo_unc * np.nansum(area_msk) * 1000 * 1000 * ndaysperyear * 1e-15
        ar_trd_reg_val = (ar_trd_reg_val.T * area_tc['common_nonnan_pixels'].data).T * 1000 * 1000 * ndaysperyear * 1e-15
        ar_trd_reg_unc = (ar_trd_reg_unc.T * area_tc['common_nonnan_pixels'].data).T * 1000 * 1000 * ndaysperyear * 1e-15
        ...
        _obs_glo_unc = np.apply_along_axis(running_mean, 0, ar_obs_glo_unc, N=12)
    ...
    fig.savefig(
        os.path.join(path_out, f"figa06_nee_iav_by_regions_in_PgCyr-1.png"),
        dpi=600,
        transparent=False,
        bbox_inches='tight'
    )
...
if __name__ == '__main__':
    plot_figa06_nee_iav_by_regions_in_PgCperyr(path_expOutput=argv[1])
# %%
