...
def process_trendy_calc_regional_msciav_koeppengeiger5ns():
    ...
    ds_studyarea_mask = ds_studyarea_mask.sortby('lon', ascend- ing=True)
    ...
    for p in range(len(dict_period['date_start'])):
        ...
        for v in range(len(dict_src)):
            ...
            for m in range(nmembers):
                ...
                ds_rm_temp = apply_koeppengeiger5ns_mask(
            ...
            # save
            file_out = os.path.join(path_trendy, f'trendyv9_S2_{_vname}-regionalMSCIAV_{date_label}_koeppengeiger5ns.nc')
            ds_rm.to_netcdf(file_out)

            print(f'saved: {file_out}', flush=True)

if __name__ == '__main__':
    process_trendy_calc_regional_msciav_koeppengeiger5ns()