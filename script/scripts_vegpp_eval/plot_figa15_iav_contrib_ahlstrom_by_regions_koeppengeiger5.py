...
def plot_fig_iav_contrib_ahlstrom_by_regions_koeppengeiger5(path_expOutput):
    ...
    ar_cov_nee_det_kg_med_oco2 = np.load(path_npz_med_oco2)['ar_contrib_det']

    # members
    path_npz_mem_oco2 = os.path.join(path_oco2, 'koeppengeiger5_region_contrib_ahlstrom_members.npz')
    ar_cov_nee_det_kg_mem_oco2 = np.load(path_npz_mem_oco2)['ar_contrib_det']
    ...
    ar_cov_gpp_det_kg_mem_trd = np.load(path_npz_mem_trd)['ar_contrib_det']
    ...
    ar_cov_reco_det_kg_mem_trd = np.load(path_npz_mem_trd)['ar_contrib_det']
    ...
    ar_cov_nee_det_kg_mem_trd = np.load(path_npz_mem_trd)['ar_contrib_det']
    ar_cov_nee_det_kg_mem_trd[idx_model2exclude, :] = np.nan  # remove jules-es-1p0, lpj-guess, ocn

    #%% create dataframe for boxplot

    # gpp
    df_mem_gpp_kg_flx_dummy = pd.DataFrame(  # a dummy df to hold the half space on the left
        {
            'region_group':np.repeat('kg5', ar_cov_gpp_det_kg_mem_trd.size),
            'region':rnames_short*ar_cov_gpp_det_kg_mem_trd.shape[0],
            'product':np.repeat('FLUXCOM', ar_cov_gpp_det_kg_mem_trd.size),
            'contrib':np.repeat(np.nan, ar_cov_gpp_det_kg_mem_trd.ravel().size)
        },
    ...
    df_mem_gpp = pd.concat([df_mem_gpp_kg_flx_dummy, df_mem_gpp_kg_trd])
    ...
    df_mem_reco_kg_flx_dummy = pd.DataFrame(  # a dummy df to hold the half space on the left
        {
            ...
            'product':np.repeat('FLUXCOM', ar_cov_reco_det_kg_mem_trd.size),
            'contrib':np.repeat(np.nan, ar_cov_reco_det_kg_mem_trd.ravel().size)
        },
        columns=['region_group', 'region', 'product', 'contrib']
    )
    ...
    bx_kg.set_xticklabels(['']*len(rnames), rotation=35)
    jt_kg = sns.stripplot(
        ax=ax,
        data=df_mem_reco.loc[df_mem_reco['region_group']=='kg5'].reset_index(),
        x='region',
    ...
    fig.supxlabel('Regions', x=0.50, y=-0.2, fontsize=16)
    ...
    save_name = os.path.join(path_out, f"fig_contrib_ahlstrom_iav_region_koeppengeiger5.png")
    fig.savefig(
        save_name,
        dpi=600,
        bbox_inches='tight',
...
if __name__ == '__main__':
    plot_fig_iav_contrib_ahlstrom_by_regions_koeppengeiger5(path_expOutput=argv[1])
