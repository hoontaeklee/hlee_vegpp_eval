...
for i in range(len(list_vname)):
    ...
    # remain target grids only
    ds_sin = ds_sin.where(ds_opti904_mask.opti904)
    ds_obs = ds_obs.where(ds_opti904_mask.opti904)
    ...
    lf_msk = np.where(opti904_mask, lf, np.nan)
    ...
    # save
    np.savez(
        f'/Net/Groups/BGI/people/hlee/hlee_vegpp_eval/data/performance_metrics_gridwise_transcomRegional_optigrids_{vname}.npz',
