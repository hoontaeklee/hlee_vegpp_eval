...
def plot_fig_contrib_ahlstrom_grids(path_expOutput):
    '''
    ...
    list_npz = [e for e in list_npz if 'ahlstrom' in e]
    list_npz = [e for e in list_npz if 'region' not in e]
    list_keys = [e.split('/')[-1].split('.')[0].replace('contrib_ahlstrom_', '') for e in list_npz]
    list_keys.sort()
    list_npz.sort()
    ...
    for i, k in enumerate(keys_to_use):  
        ...
        ar_contrib = npz['contrib']
        ar_contrib = np.where(common_nonnan, ar_contrib, np.nan)
        ...
        im = ax1.imshow(ar_contrib, transform=ccrs.PlateCarree(), vmin=-0.001, vmax=0.001, cmap='coolwarm', aspect='auto')
    ...
    save_name = os.path.join(path_out, f"fig_contrib_ahlstrom_grids.png")
    fig.savefig(
        save_name,
        dpi=600,
        bbox_inches='tight',
...
if __name__ == '__main__':
    plot_fig_contrib_ahlstrom_grids(path_expOutput=argv[1])
