...
def plot_vegpp_gridwise_performance(path_expOutput):
    ...
    for vname in list_vname:
        path_metrics_temp = os.path.join(path_src, f'data/performance_metrics_gridwise_transcomRegional_{vname}.npz')
    ...
    path_out = os.path.join(path_src, 'figures_revision1')
    ...
    fig.text(-0.03, 0.51, r'$R^2$', va='center', rotation='vertical', fontsize=fsize_suplab)

    fig.savefig(
        os.path.join(path_out, f"fig03_gridwise_performance_rsq_transcom.png"),
        dpi=600,
        transparent=False,
        bbox_inches='tight'
    )
...
if __name__ == '__main__':
    plot_vegpp_gridwise_performance(path_expOutput=argv[1])
# %%
