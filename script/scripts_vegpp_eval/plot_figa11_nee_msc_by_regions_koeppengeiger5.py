...
def plot_fig_nee_msc_by_regions_koeppengeiger5(path_expOutput):
    ...
    ds_oco2_reg_mad = xr.open_dataset(os.path.join(path_oco2, 'EnsMAD_LNLGIS_GFED4FireRemoved_Koeppengeiger5Regions_2015_2019_studyArea_msc.nc'))
    ...
    fig.text(-0.16, 0.16, r'NEE', va='center', rotation='vertical', fontsize=fsize_suplab)
    fig.text(-0.16, 0.33, r'(PgC yr$^{-1}$)', va='center', rotation='vertical', fontsize=fsize_suplab, color='g')
    fig.text(-0.16, 0.67, r' or (gC m$^{-2}$ day$^{-1}$)', va='center', rotation='vertical', fontsize=fsize_suplab)

    # add legends and alphabets
    axes[0].legend(
                handles=legend_handles,  # [l1, l2, l3, l4],
                loc='upper center', bbox_to_anchor=(0.5, 1.13), bbox_transform=fig.transFigure,
    ...
    fig.savefig(
        os.path.join(path_out, f"fig_nee_msc_by_regions_kg5.png"),
        dpi=600,
        transparent=False,
        bbox_inches='tight'
    )
...
if __name__ == '__main__':
    plot_fig_nee_msc_by_regions_koeppengeiger5(path_expOutput=argv[1])
# %%
