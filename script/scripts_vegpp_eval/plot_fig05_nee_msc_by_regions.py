'''
figure 05: nee seasonal variability across regions
'''
...
def plot_fig05_nee_msc_by_regions(path_expOutput):
    ...
    # path_expOutput = os.path.join(path_bgi, 'people/hlee/sindbad/data/output/VEGPP2pool1519gppActmin_studyArea_10k_1_20250108/detrended')
    # path_expOutput = os.path.join(path_bgi, 'people/hlee/sindbad/data/output/VEGPP2pool1519gppActmin_studyArea_10k_1_20250101/detrended')
    ...
    path_out = os.path.join(path_bgi, 'people/hlee/hlee_tws_nee_in_model/figures/test_gppActmin')
    ...
    # ds_oco2_glo_med = xr.open_dataset(os.path.join(path_oco2, 'EnsMedian_global_mean_fluxes_LNLGIS_GFED4FireRemoved_2015_2019_studyArea_msc.nc'))
    ds_oco2_glo_mad = xr.open_dataset(os.path.join(path_oco2, 'EnsMAD_global_mean_fluxes_LNLGIS_GFED4FireRemoved_2015_2019_studyArea_msc.nc'))
    # ds_oco2_reg_med = xr.open_dataset(os.path.join(path_oco2, 'EnsMedian_LNLGIS_GFED4FireRemoved_TranscomRegions_2015_2019_studyArea_msc.nc'))
    ds_oco2_reg_mad = xr.open_dataset(os.path.join(path_oco2, 'EnsMAD_LNLGIS_GFED4FireRemoved_TranscomRegions_2015_2019_studyArea_msc.nc'))
    
    # ds_oco2_glo_med = ds_oco2_glo_med.isel(time=range(12))
    ds_oco2_glo_mad = ds_oco2_glo_mad.isel(time=range(12))
    # ds_oco2_reg_med = ds_oco2_reg_med.isel(time=range(12))
    ds_oco2_reg_mad = ds_oco2_reg_mad.isel(time=range(12))

    ar_obs_glo_unc = ds_oco2_glo_mad[f'net_mad_nf_{_tsc}'].values / 365 * 1.25  # mad to std
    ar_obs_reg_unc = ds_oco2_reg_mad[f'net_mad_nf_{_tsc}'].values / 365 * 1.25  # mad to std

    ##
    # load trendy
    path_trendy = os.path.join(path_bgi, 'people/hlee/data/trendy/v9/regridded_1deg')
    ds_trd_nee_reg = xr.open_dataset(os.path.join(path_trendy, 'trendyv9_S2_nee-regionalMSCIAV_2015-2019_transcom.nc'))
    ds_trd_nee_reg = ds_trd_nee_reg.isel(time=range(12))
    ...
    for i in range(len(list_var)):
        ...
        _nc_obs_tc = apply_transcom_mask(
            dsin=_nc_obs,
            pathlf=files_lf[_vname][0] if _vname in files_lf.keys() else 'ones',
            varlf=files_lf[_vname][1] if _vname in files_lf.keys() else 'landfraction',
            faclf=files_lf[_vname][2] if _vname in files_lf.keys() else '1.0',
        ...
        _ar_wtm_temp = _nc_obs_tc[_vname_in_obs].values.T
        ...
        mask_trd = np.ones_like(ar_trd_glo, dtype=bool)
        mask_trd[idx_model2exclude, :] = False
        ar_trd_glo = ar_trd_glo[mask_trd].reshape(-1, mask_trd.shape[1])
        ...
        ar_trd_reg_val = np.nanmedian(ds_trd_nee_reg.isel(model=mask_trd[:, 0])[f'nee_{_tsc}'].values, axis=0) * 1000 * 86400  # kgC m-2 s-1 to gC m-2 d-1
        ar_trd_reg_unc = calc_mad(ds_trd_nee_reg.isel(model=mask_trd[:, 0])[f'nee_{_tsc}'].values, axis=0) * 1000 * 86400 * 1.25 # kgC m-2 s-1 to gC m-2 d-1; mad to std (*1.25)
        ...
        ylim_rng[0] = np.nanmin(
            ...
            np.nanmin(_nc_obs_tc[_vname_in_obs]) * sc_yrng,
            # np.nanmin(ar_obs_reg_val) * sc_yrng,
            np.nanmin(_sin) * sc_yrng,
            np.nanmin(_obs) * sc_yrng,
            # np.nanmin(ar_obs_glo_val) * sc_yrng,
            # np.nanmin(ar_car_reg_val) * sc_yrng,
        ...
        ylim_rng[1] = np.nanmax(
            ...
            np.nanmin(_nc_obs_tc[_vname_in_obs]) * sc_yrng,
            # np.nanmax(ar_obs_reg_val) * sc_yrng,
            np.nanmax(_sin) * sc_yrng,
            np.nanmin(_obs) * sc_yrng,
            # np.nanmax(ar_obs_glo_val) * sc_yrng,
            # np.nanmax(ar_car_reg_val) * sc_yrng,
        ...
        ax_y2.spines["left"].set_position(("axes", -0.13)) # green one
        ...
        for r in np.arange(1, 12):
            ...
            _obs = _nc_obs_tc.sel(region=r)[_vname_in_obs].values  # ar_obs_reg_val[r-1]
            _obs_unc = ar_obs_reg_unc[r-1]
            ...
            ax.fill_between(x_years, _obs-_obs_unc[r-1], _obs+_obs_unc[r-1], color=dict_colors['obs'][i], alpha=area_alpha)
    ...
    fig.text(0.5, -0.08, 'Months', ha='center', fontsize=fsize_suplab)
    fig.text(-0.1, 0.26, r'NEE', va='center', rotation='vertical', fontsize=fsize_suplab)
    fig.text(-0.1, 0.39, r'(PgC yr$^{-1}$)', va='center', rotation='vertical', fontsize=fsize_suplab, color='g')
    fig.text(-0.1, 0.64, r' or (gC m$^{-2}$ day$^{-1}$)', va='center', rotation='vertical', fontsize=fsize_suplab)

    # add legends and alphabets
    axes[0].legend(
                handles=legend_handles,  # [l1, l2, l3, l4],
                loc='upper center', bbox_to_anchor=(0.5, 1.12), bbox_transform=fig.transFigure,
    ...
    exp_name_date = path_expOutput.split('/')[-2]
    fig.savefig(
        os.path.join(path_out, f"fig06_nee_msc_by_regions_{exp_name_date}.png"),
        dpi=600,
        transparent=False,
        bbox_inches='tight'
    )
...
if __name__ == '__main__':
    plot_fig05_nee_msc_by_regions(path_expOutput=argv[1])
# %%
