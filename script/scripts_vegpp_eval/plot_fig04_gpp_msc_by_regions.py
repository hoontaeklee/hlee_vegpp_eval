'''
figure 04: gpp seasonal variability across regions
'''
...
def plot_fig04_gpp_msc_by_regions(path_expOutput):
    ...
    # --> below after aggregating regional time series
     ##
    ...
    for i in range(len(list_var)):
        ...
        ar_trd_reg_val = np.nanmedian(ds_trd_reg.isel(model=mask_trd[:, 0])[f'gpp_{_tsc}'].values, axis=0) * 1000 * 86400  # kgC m-2 s-1 to gC m-2 d-1
        ar_trd_reg_unc = calc_mad(ds_trd_reg.isel(model=mask_trd[:, 0])[f'gpp_{_tsc}'].values, axis=0) * 1000 * 86400 * 1.25 # kgC m-2 s-1 to gC m-2 d-1; mad to std (*1.25)
        ...
        ylim_rng[0] = np.nanmin(
            ...
            np.nanmin(ar_flx_glo_val) * sc_yrng,
        ...
        ylim_rng[1] = np.nanmax(
            ...
            np.nanmax(ar_flx_glo_val) * sc_yrng,
        ...
        ax_y2.spines["left"].set_position(("axes", -0.12)) # green one
        ...
        for r in np.arange(1, 12):
            ...
            _obs = _nc_obs_tc.sel(region=r)[_vname_in_obs].values
    ...
    fig.text(-0.1, 0.26, r'GPP', va='center', rotation='vertical', fontsize=fsize_suplab)
    ...
    fig.savefig(
        os.path.join(path_out, f"fig04_gpp_msc_by_regions_{exp_name_date}.png"),
        dpi=600,
        transparent=False,
        bbox_inches='tight'
    )
...
if __name__ == '__main__':
    plot_fig04_gpp_msc_by_regions(path_expOutput=argv[1])
# %%
