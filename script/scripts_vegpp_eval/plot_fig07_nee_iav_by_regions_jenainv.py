'''
plot vegpp carbon fluxes by transcom regions

- NEP and GPP MSC
- NEP IAV running mean
- GPP and NEP raw signals
'''
...
def plot_figa07_nee_iav_by_regions_jenainv(path_expOutput):
    ...
    # path_expOutput = '/Net/Groups/BGI/people/hlee/sindbad/data/output/VEGPP2pool1519_studyArea_10k_RD4wtnanmean_jeni_1_20240314/detrended'
    ...
    if isdet:
        ...
        # var_in_sindbad: [var_in_data, unit, period, conversion_factor_to_daily path_to_data]
            ...
            'NEE20012019': [f'NEE_{_tsc}', 'gC m-2 day-1', ('2001-01-01', '2019-12-31'), 1.0, os.path.join(path_bgi, 'people/hlee/data/jenainv/s99oc_v2022_daily/NEE.monthly.2001-2019.1deg.3dim.global.det.nc')]
        }
    else:
        files_obs = {
    ...
    for i in range(len(list_var)):
        ...
        if _vname=='NEE20012019':
            ...
            # l3, = ax.plot(x_years, _car, linestyle=dict_linestyle['car'][i], color=dict_colors['car'][i], alpha=line_alpha, label='CARDAMOM')
        ...
        if _vname=='NEE20152019':
            _obs = np.concatenate((np.repeat(np.nan, 168), _obs))
            _obs_glo_unc = np.apply_along_axis(running_mean, 0, ar_oco2_glo_unc, N=12)
            _obs_glo_unc = np.concatenate((np.repeat(np.nan, 168), _obs_glo_unc))
            
            l5, = ax.plot(x_years, _obs, linestyle=dict_linestyle['oco2'][i], color=dict_colors['oco2'][i], alpha=line_alpha, label='OCO2')
            ax.fill_between(x_years, _obs-_obs_glo_unc, _obs+_obs_glo_unc, linestyle=dict_linestyle['oco2'][i], color=dict_colors['oco2'][i], alpha=area_alpha)
            legend_handles.append(l5)
        
        ax_y2 = ax.twinx()
        ax_y2.tick_params(axis='y', colors='g')
        ax_y2.spines["left"].set_position(("axes", -0.2)) # green one
        ...
        for r in np.arange(1, 12):
            ...
            # _car = ar_car_reg_val[(r-1)]
            _obs_unc = np.apply_along_axis(running_mean, 0, ar_oco2_reg_unc[(r-1), :], N=12)
            ...
            if _vname=='NEE20012019':
                ...
                # p3 = ax.plot(x_years, _car, linestyle=dict_linestyle['car'][i], color=dict_colors['car'][i], alpha=line_alpha, label='CARDAMOM')
            ...
            if _vname=='NEE20152019':
                _obs = np.concatenate((np.repeat(np.nan, 168), _obs))
                _obs_unc = np.concatenate((np.repeat(np.nan, 168), _obs_unc))

                p1 = ax.plot(x_years, _obs, linestyle=dict_linestyle['oco2'][i], color=dict_colors['oco2'][i], alpha=line_alpha, label='OCO2')
                ax.fill_between(x_years, _obs-_obs_unc, _obs+_obs_unc, linestyle=dict_linestyle['oco2'][i], color=dict_colors['oco2'][i], alpha=area_alpha)
    ...
    fig.savefig(
        os.path.join(path_out, f"figa07_nee_iav_by_regions_jenainv.png"),
        dpi=600,
        transparent=False,
        bbox_inches='tight'
    )
...
if __name__ == '__main__':
    plot_figa07_nee_iav_by_regions_jenainv(path_expOutput=argv[1])
# %%
