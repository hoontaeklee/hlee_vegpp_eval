...
def process_trendy_calc_regional_msciav_transcom():
    path_trendy = os.path.join(path_bgi, f'people/hlee/data/trendy/v9/regridded_1deg/detrended')
    ...
    for p in range(len(dict_period['date_start'])):
        ...
        for v in range(len(dict_src)):
            ...
            ds_tc = ds_tc.expand_dims(dim={'region': np.arange(1, 12, 1)}, axis=1)
            ...
            for m in range(nmembers):
                ...
                ds_tc_temp = apply_transcom_mask(
                    ...
                    tosave=False,
                    toplot=False)
                ar_bowl_msc[m, :, :] = ds_tc_temp[f'{_vname}_msc'][f'{_vname}_msc'].values
                ar_bowl_iav[m, :, :] = ds_tc_temp[f'{_vname}_det'][f'{_vname}_det'].values
            ...
            # save
            file_out = os.path.join(path_trendy, f'trendyv9_S2_{_vname}-regionalMSCIAV_{date_label}_transcom.nc')
            ds_tc.to_netcdf(file_out)

            print(f'saved: {file_out}', flush=True)

if __name__ == '__main__':
    process_trendy_calc_regional_msciav_transcom()