...
def calc_vegpp_cov_matrix_by_regions_koeppengeiger5(path_expOutput):
    ...
    for i, var_to_diag in enumerate(vars_to_diag):
        ...
        # mask out heavily cultivated grid cells
        ds_msc_sin = ds_msc_sin.where(~ds_lc.landcover)
        ds_det_sin = ds_det_sin.where(~ds_lc.landcover)
        ...
        np.savez(
            os.path.join(path_expOutput, f'koeppengeiger5_region_cov_norm_{var_to_diag}_withoutCroplandGrids.npz'),
