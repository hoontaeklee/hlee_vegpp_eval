...
def process_trendy_calc_regional_msciav_koeppengeiger5():
    ...
    ar_area_msk = np.where(ar_fcc, np.nan, ar_area_msk)
    ...
    for p in range(len(dict_period['date_start'])):
        ...
        for v in range(len(dict_src)):
            ...
            ds_temp = ds_temp.where(~ds_fcc.treecoverchange)
            ...
            # save
            file_out = os.path.join(path_trendy, f'trendyv9_S2_{_vname}-regionalMSCIAV_{date_label}_koeppengeiger5_withoutDeforestedGrids.nc')
